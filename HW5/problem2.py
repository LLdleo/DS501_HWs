...
def sample(n):
    ...
    s_c = np.copy(n.s)
    x = n.x
    while len(get_valid_moves(s_c)) > 0 and check_game(s_c) not in [0, 1, -1]:
        m = get_valid_moves(s_c)
...
def expand(n):
    ...
    for r, c in get_valid_moves(n.s):
        s_new = np.copy(n.s)
        s_new[r][c] = n.x
        child_node = MCNode(s_new, x=-n.x, p=n, m=tuple([r, c]))
        n.c.append(child_node)
    i = np.random.randint(0, len(n.c))
    c = n.c[i]
    #########################################
    return c
...
def select_a_child(p):
    ...
    highest = -1000
    for child in p.c:
        ucb = compute_UCB(child.v, child.N, N, x)
        if ucb > highest:
            highest = ucb
            c = child
    #########################################
