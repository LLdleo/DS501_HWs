...
def check_game(s):
    ...
    for line in line_list:
        line_content = [s[line[0][0]][line[0][1]], s[line[1][0]][line[1][1]], s[line[2][0]][line[2][1]]]
...
class PlayerRandom:
    ...
    def play(self, s, x=1):
        ...
        rdm_move = np.random.randint(0, len(valid_moves))
        r, c = valid_moves[rdm_move]
        #########################################
        return r, c
...
class Node:
    ...
    def __init__(self, s, x=1, p=None, c=None, m=None, v=None):
        ...
        self.m = m  # the move that it takes from the parent node to reach this node. m is a tuple (r,c), r:row of move, c:column of the move
    ...
    def expand(self):
        ...
        for r, c in next_moves:
            ...
            s_new[r][c] = self.x
            new_m = tuple([r, c])
            child_node = Node(s_new, x=-self.x, p=self, m=new_m)
