...
def load_batting(filename='Batting.csv'):
    ...
    X = pd.read_csv(filename, dtype={'H': int, 'AB': int})
    #########################################
    return X 
...
def aggregate_batting(X):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    column_order = X.columns.values.tolist()
    column_order.remove('teamID')
    column_order.remove('lgID')
    Y = X[column_order].groupby('playerID').sum().reset_index()
    Y['yearID'] = 2001
    #########################################
    return Y 
...
def join_batting(X,Y):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    Z = X.merge(Y, left_on='playerID', right_on='playerID')
    #########################################
    return Z 
