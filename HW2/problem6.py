...
def sum_salaries(T, D):
    ...
    S = D[D['playerID'].isin(T)]['salary'].sum()
    #########################################
    return S
...
def scout():
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    T = ['giambje01', 'hattesc01', 'justida01']
    #########################################
    return T
...
def rank_BA(D, t=300,max_salary=1200000):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    # D['BA'] = D[(D['AB'] >= t) & (D['salary'] <= max_salary)]['H'] / D[(D['AB'] >= t) & (D['salary'] <= max_salary)]['AB']
    D['BA'] = np.where((D['AB'] >= t) & (D['salary'] <= max_salary), D['H'] / D['AB'], 0)
    R = D.sort_values('BA', ascending=False)['playerID'].tolist()
    #########################################
    return R
...
def rank_OBP(D,t=300,max_salary=1200000):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    D['OBP'] = np.where((D['AB'] >= t) & (D['salary'] <= max_salary), (D['H'] + D['BB'] + D['HBP']) / (D['AB'] + D['BB'] + D['HBP'] + D['SF']), 0)
    R = D.sort_values('OBP', ascending=False)['playerID'].tolist()
    #########################################
    return R 
