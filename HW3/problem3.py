...
def cosine_similarity(X, Y):
    ...
    S = np.dot(X, Y) / (math.sqrt(np.sum(X**2))*math.sqrt(np.sum(Y**2)))
    #########################################
    return S 
...
def pairwise_item_sim(R):
    ...
    ## INSERT YOUR CODE HERE
    line_num = R.shape[0]
    S = np.zeros([line_num, line_num])
    for index1, line1 in enumerate(R):
        for index2, line2 in enumerate(R):
            l1n = np.where(line1 != None)[0]
            l2n = np.where(line2 != None)[0]
            c = list(set(l1n) & set(l2n))
            line1_new = line1[c]
            line2_new = line2[c]
            S[index1, index2] = cosine_similarity(line1_new, line2_new)
    #########################################
    return S 
...
def predict(R,S,i,j):
    ...
    row_i = R[:, j]
    s_i = S[i, :]
    rn = np.where((row_i != 0) & (row_i != None))[0]
    sn = np.where(s_i != None)[0]
    cr = list(set(rn) & set(sn) - {i})
    r_new = row_i[cr]
    s_new = s_i[cr]
