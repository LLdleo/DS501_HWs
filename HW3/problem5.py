...
def compute_dU(L,U, V,mu=1.):
    ...
    ## INSERT YOUR CODE HERE
    row_num = L.shape[0]
    # dU = 2 * mu * U - 2 * L.sum(axis=1).reshape([row_num, 1])
    print(L.shape)
    print(V.shape)
    dU = 2 * mu * U - 2 * np.dot(L, V.T)
    # print(dU)
    #########################################
    return dU 
...
def compute_dV(L,U, V, mu=1.):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    # dV = 2 * mu * V - 2 * L.sum(axis=0)
    dV = 2 * mu * V - 2 * np.dot(U.T, L)
    #########################################
    return dV 
...
def matrix_decoposition(R, k=5, max_steps=1000000, beta=.01, mu=.01):
    ...
    for _ in range(max_steps): 
        ...
        # U = U_new
        if not np.allclose(U, U_new, atol=.000001):
            U = U_new
        else:
            break
        ...
        # V = V_new
        if not np.allclose(V, V_new, atol=.000001):
             V = V_new
        else:
            break
        #########################################
